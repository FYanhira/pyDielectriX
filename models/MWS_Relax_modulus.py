...
class MWSModulusModel(BaseModel):
    def __init__(self):
        ...
        self.params_init = {
            ...
            'M_s2': (None, None, None), 
            'tau_alpha': (1e-5, 1e-8, 1e-1),    # valor inicial y rango fijos
            'tau_beta': (1e-5, 1e-8, 1e-1),
            'tau_gamma': (1e-5, 1e-8, 1e-1),
            'alpha': (0.5, 0.0, 1.0),
            'beta': (0.5, 0.0, 1.0),
            'gamma': (0.5, 0.0, 1.0),
        }
    ...
    def set_auto_params_from_data(self, M_real, n_points=5):
        ...
        avg_mid_low    = np.mean(M_real[-n_points:])       # intermedia-baja
        avg_mid_high   = np.mean(M_real[-n_points:])       # intermedia-alta
